...
const createReport = async(req,res,next)=>{
    ...
    const {from,to,title,description,docs} = req.body
    ...
    var contract = new web3.eth.Contract(CONTRACT_ABI, CONTRACT_ADDRESS);
    try{
        var receipt = await contract.methods.createReport(from,to,title,description,date,docs).send({from: process.env.ACCOUNT_ADDRESS});
...
const addIntoRecords = async(req,res,next)=>{
    ...
    var contract = new web3.eth.Contract(CONTRACT_ABI, CONTRACT_ADDRESS);
    try{
        var receipt = await contract.methods.addIntoRecords(id,docId).send({from: process.env.ACCOUNT_ADDRESS});
    }catch(err){
        provider.engine.stop();
        const error = new HttpError(
...
const addAgent=async(req,res,next)=>{
    ...
    let hospital;
    try {
        hospital = await contract.methods.getHospital(id1).call();
    ...
    try{
        var receipt = await contract.methods.addAgent(id1,id2).send({from: process.env.ACCOUNT_ADDRESS});
    }catch(err){
        provider.engine.stop();
        const error = new HttpError(
